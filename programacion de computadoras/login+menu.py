    ...
    def quitar_del_carrito(product_name):
        ...
        for i, item in enumerate(carrito):
            if item["nombre"] == product_name:
                #devolver el producto al stock general
                for p in productos_disponibles:
                    if p["nombre"] == product_name:
                        p["stock"] += 1
                        break
                carrito.pop(i) #sacar del carrito
                found = True
        ...
        go_carrito() #refrescar la vista de tu carrito
    ...
    def pagar_con_opciones(total_a_pagar):
        ...
        def finalizar_pago():
            ...
            if metodo_seleccionado in ["Tarjeta Visa", "Mastercard"]:
                #tomar los valores de los campos de la tarjeta
...
#agrgar al acrrito y modificarsu stock
    def agregar_carrito(prod_to_add):
        #buscar el producto real en nuestra lista para modificar su stock
        found_product = None
        for p in productos_disponibles:
    ...
    def mostrar_estadisticas_admin():
        ...
        # Convertimos los datos a un formato que Pandas entiende
